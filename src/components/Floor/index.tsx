 ...
 * @LastEditTime: 2023-01-01 17:03:34
...
function Floor({ warriorPosition, floor, unitSize, updateWarrior }: Props) {
  const [floors, { update: updateFloors }] = store.useModel('floors')

  const { walls, weakWalls, monsters, doors, stairs, articles, npc } = floors[floor]
  const currentFloor = floors[floor]
  const handleFloorClick = (e: TouchEvent) => {
    const [x, y] = warriorPosition as Position
  ...
  const allArticles: ArticleType[] = [...walls, ...weakWalls, ...articles, ...doors, ...stairs, ...monsters, ...npc]
    ...
    <View
      ...
      <>
        {allArticles.map(article => (
          <Article id={article.id} name={article.name} positions={article.positions} key={article.id} unitSize={unitSize} />
        ))}
      </>
    </View>
  );
